...
public:
    MPCReadTraj(){
        ...
        //sub_current_pose_ = nh.subscribe("/info",1,&MPCReadTraj::callbackconverter,this);
        closest_idx_ = 0;  
...
void Read_traj( ){
    std::ifstream myfile("/home/nvidia/suv_car/mpc_traj3.txt");
    if(myfile.fail()){
        ROS_INFO("read failed");
        return ;

    }
    //ROS_INFO("Data: %d",myfile.eof());
    while(!myfile.eof()){
        char buffer[1000];
        ...
        //ROS_INFO("%lf %lf %lf %lf %lf %lf %lf %d %lf",temp_waypoint.pose.pose.position.x,temp_waypoint.pose.pose.position.y,temp_waypoint.pose.pose.position.z,temp_waypoint.pose.pose.orientation.x,temp_waypoint.pose.pose.orientation.y,temp_waypoint.pose.pose.orientation.z,temp_waypoint.pose.pose.orientation.w,temp_waypoint.direction,temp_waypoint.twist.twist.linear.x);
...
void callbackconverter(const geometry_msgs::PoseStamped &msg){
    ...
    ROS_INFO("size:%d",(int)base_waypoints_.waypoints.size());
    //寻找最近点
    int closest_idx = -1;
    ...
    ROS_INFO("size1:%d",(int)mpc_waypoints.waypoints.size());
    ...
    ROS_INFO("size:%d",(int)mpc_waypoints.waypoints.size());
    pub_waypoints_.publish(mpc_waypoints);

}
//调试用
// void callbackconverter(const mpc_msgs::ControlCommand &msg){

// ROS_INFO_STREAM("start mpc");
// pub_waypoints_.publish(base_waypoints_);
