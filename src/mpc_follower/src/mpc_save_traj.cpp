...
private:
    ...
    void callbackGnssPose(const geometry_msgs::PoseStamped &msg)
    {
        ...
        if (sq_dist(veh_pose.pose.position, front_pose.pose.position) >= 0.5)
        {
            ...
            if (count1 != count2)
            {
                ...
                //waypoint_temp.twist.twist.linear.x = amathutils::kmph2mps(status.speed);
                waypoint_temp.twist.twist.linear.x = 1;
                count3++;
                std::ofstream out("/home/nvidia/exper/mpc_traj3.txt", std::ios::app);
                const double current_yaw = tf2::getYaw(waypoint_temp.pose.pose.orientation);
                ...
                out << waypoint_temp.pose.pose.position.x << ' ' << waypoint_temp.pose.pose.position.y << ' ' << waypoint_temp.pose.pose.position.z << ' ' << current_yaw << ' ' << waypoint_temp.twist.twist.linear.x << std::endl;
