...
bool MPCFollower::calculateMPC(double &vel_cmd, double &acc_cmd, double &steer_cmd, double &steer_vel_cmd)
{
  ...
  ROS_INFO("nearest_index: %d",(int)nearest_index);
  acc_cmd = ref_traj_.direction[nearest_index];
  ROS_INFO("direction: %lf",acc_cmd);
...
void MPCFollower::callbackRefPath(const mpc_msgs::Lane::ConstPtr &msg)
{
  ...
  //ROS_INFO("(current_waypoints_.waypoints.at(0).direction:%d",(int)current_waypoints_.waypoints.at(0).direction);
  if(current_waypoints_.waypoints.at(0).direction == 3){
    for(int i = 0; i < (int)current_waypoints_.waypoints.size();i++){
      if(current_waypoints_.waypoints[i].direction==3){
        double x = vehicle_status_.pose.position.x * 2 - current_waypoints_.waypoints[i].pose.pose.position.x;
        double y =  vehicle_status_.pose.position.y * 2 - current_waypoints_.waypoints[i].pose.pose.position.y ;
        current_waypoints_.waypoints[i].pose.pose.position.x  = x;
        current_waypoints_.waypoints[i].pose.pose.position.y = y;
              //ROS_INFO("basewaypoints.pose.position.x:%lf,basewaypoints_pose.pose.position.y:%lf",base_waypoints_.waypoints[i].pose.pose.position.x,base_waypoints_.waypoints[i].pose.pose.position.y);
        }
    }
  }
  ...
  MPCTrajectory traj;
  if(current_waypoints_.waypoints.at(0).direction == 6){
    traj.direction = {6};
    ref_traj_ = traj;
    return;
  }
  
  //ROS_INFO("current_waypoints_.waypoints_size: %d",(int)current_waypoints_.waypoints.size());
  ref_traj_ = traj;
  //ROS_INFO("ref_traj_.vx.size: %d",(int)ref_traj_.direction);
  // for(int i=0;i<(int)ref_traj_.vx.size();i++)
  // {
  //   int x = ref_traj_.vx[i];

  //   ROS_INFO("-----------ref_vx:%d---------",x);
  
  // }

  //ROS_INFO("-----------(ref_traj_.direction:%d   traj.direction:%d",(int)ref_traj_.direction,(int)traj.direction);
  ...
  for(int i = 0; i < (int)ref_traj_.vx.size();i++)
  {
    int dire = current_waypoints_.waypoints[0].direction;
    if (i > (int)ref_traj_.vx.size()-5)
    {
      ref_traj_.direction.push_back(6);
      continue;
    }
    ref_traj_.direction.push_back(dire);

  }
  ROS_INFO("traj.size() : %d",(int)ref_traj_.vx.size());
  ROS_INFO("ref_traj_.direction: %d",(int)ref_traj_.direction.size());
  //ROS_INFO("ref_traj_.direction: %d",(int)ref_traj_.direction[ref_traj_.vx.size()-2]);
  // for(int i=0;i<(int)ref_traj_.vx.size();i++)
  // {
  //   int x = ref_traj_.vx[i];
...
void MPCFollower::callbackVehicleStatus(const mpc_msgs::VehicleStatus &msg)
{
  ...
  vehicle_status_.tire_angle_rad = msg.steer/14.6/180*3.1415;  //当前转向角
...
void MPCFollower::publishTwist(const double &vel_cmd, const double &omega_cmd)
{
  ...
  twist.twist.linear.x = vel_cmd; //目标纵向速度 m/s
...
void MPCFollower::publishCtrlCmd(const double &vel_cmd, const double &acc_cmd, const double &steer_cmd)
{
  ...
  // cmd.linear_velocity = vel_cmd; //目标纵向速度 m/s
  ...
  // }
  //该处的acc_cmd值表示轨迹方向
  cmd.linear_acceleration = acc_cmd;
  if((int)acc_cmd == 3){
    cmd.linear_velocity = -vel_cmd;
    //cmd.linear_acceleration = -acc_cmd;
    // cmd.cmd.steering_angle = (-steer_cmd+3.1415926835897931*0.5986/180)/0.9039;
    cmd.steering_angle = -steer_cmd;
  }
  else if((int)acc_cmd == 6){
    cmd.linear_velocity = 0;
    //cmd.linear_acceleration = 0;
    cmd.steering_angle = 0;
  }
